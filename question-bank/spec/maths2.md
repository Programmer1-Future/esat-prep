# ESAT Content Specification — Mathematics 2
...
- **Note on fidelity:** The Mathematics 2 appendix is dense with stacked fractions, superscripts, radicals and Greek letters that the PDF text extractor flattened or reordered. The prose/spec-point text itself is verbatim. A few worked-example expressions (MM1.2 surd examples, MM2.4 binomial notation, MM4.6 trig equation examples, MM5.2–5.3 log examples, MM7.2/7.4 integration examples) came through garbled; each is marked inline and the standard typeset form reconstructed for readability — consult the source PDF (Appendix 1, Mathematics 2 pages) for the exact printed form.
- **Assumed knowledge:** Per the specification's content table, Mathematics 2 assumes the topics labelled **'M'** (all of Mathematics 1) **and** the topics labelled **'MM'** (this file). See maths1.md for the shared 'M' content and the format/units preamble.

---

## APPENDIX 1: MATHEMATICS 2 (verbatim)
...
MM1.2  Use and manipulation of surds.
   ...
   [The two worked examples were garbled by the PDF extractor — they are surd-simplification/rationalisation examples of the form 5/(3√2 − √5) and 3/(√7 − 2√3) or similar; consult the source PDF for the exact printed expressions.]
...
MM1.6  Algebraic manipulation of polynomials, including:
   ...
   b. factorisation and simple algebraic division (by a linear polynomial, including those of the form ax + b, and by quadratics, including those of the form ax² + bx + c)
...
MM1.7  Qualitative understanding that a function is a many-to-one (or sometimes just a one-to-one) mapping.
   Familiarity with the properties of common functions, including f(x) = √x (which always means the 'positive square root') and f(x) = 1/x.
   [The two function examples were rendered garbled by the extractor; reconstructed here as √x and 1/x, the standard common-function examples — verify against source PDF.]

MM2. Sequences and series
MM2.1  Sequences, including those given by a formula for the nth term and those generated by a simple recurrence relation of the form xₙ₊₁ = f(xₙ).
...
MM2.4  Binomial expansion of (1 + x)ⁿ for positive integer n, and for expressions of the form (a + f(x))ⁿ for positive integer n and simple f(x).
   The notations n! and ⁿCᵣ (i.e. "n choose r").
...
MM4. Trigonometry
MM4.1  The sine and cosine rules, and the area of a triangle in the form ½ab sin C.
...
MM4.5  Knowledge and use of the equations:
   a. tan θ = sin θ / cos θ
   b. sin² θ + cos² θ = 1
MM4.6  Solution of simple trigonometric equations in a given interval (this may involve the use of the identities in MM4.5).
   For example:
     tan x = 1/√3   for −π < x < π
     sin²(2x + π/3) = ½   for −2π < x < 2π
     12 cos² x + 6 sin x − 10 = 2   for 0° < x < 360°
   [The intervals and arguments were partly garbled by the extractor; reconstructed to the standard forms above — verify against source PDF.]

MM5. Exponentials and logarithms
MM5.1  y = aˣ and its graph, for simple positive values of a.
MM5.2  Laws of logarithms:
   a. aᵇ = c  ⇔  b = logₐ c
   b. logₐ x + logₐ y = logₐ(xy)
   c. logₐ x − logₐ y = logₐ(x/y)
   d. k logₐ x = logₐ(xᵏ)
   including the special cases:
   e. logₐ(1/x) = −logₐ x
   f. logₐ a = 1
   Questions requiring knowledge of the change of base formula will not be set.
MM5.3  The solution of equations of the form aˣ = b, and equations which can be reduced to this form, including those that need prior algebraic manipulation.
   For example:  3^(2x) = 4  and  25ˣ − 3 × 5ˣ + 2 = 0.
...
MM6.1  The derivative of f(x) as the gradient of the tangent to the graph y = f(x) at a point.
   ...
   c. Knowledge of notation: dy/dx, d²y/dx², f′(x), and f″(x).
   Differentiation from first principles is excluded.
MM6.2  Differentiation of xⁿ for rational n, and related sums and differences. This might require some simplification before differentiating.
   For example, the ability to differentiate an expression such as (3x² − 2)² / x  [expression garbled in extraction; reconstructed — verify against source PDF].
MM6.3  Applications of differentiation to gradients, tangents, normals, stationary points (maxima and minima only), strictly increasing functions [if f′(x) > 0] and strictly decreasing functions [if f′(x) < 0]. Points of inflexion will not be examined, although a qualitative understanding of points of inflexion in the curves of simple polynomial functions is expected.
...
MM7.2  Finding definite and indefinite integrals of xⁿ for n rational, n ≠ −1, and related sums and differences, including expressions which require simplification prior to integrating.
   For example:  ∫(x − 2)² dx  and  ∫(3x² + 5)/√x dx  [second example garbled in extraction; reconstructed — verify against source PDF].
MM7.3  An understanding of the Fundamental Theorem of Calculus and its significance to integration. Simple examples of its use may be required in the forms:
   a. ∫ₐᵇ f(x) dx = F(b) − F(a), where F′(x) = f(x)
   b. d/dx ∫ₐˣ f(x) dx = f(x)
MM7.4  Combining integrals with either equal or contiguous ranges.
   For example:
     ∫₂⁵ f(x) dx + ∫₂⁵ g(x) dx = ∫₂⁵ [f(x) + g(x)] dx
     ∫₂⁴ f(x) dx + ∫₄³ f(x) dx = ∫₂³ f(x) dx
   [Ranges were garbled by the extractor; reconstructed to the standard contiguous-range identities above — verify against source PDF.]
...
MM8.4  Understand how altering the values of a, b and c in y = a(x + b)² + c affects the corresponding graph.
MM8.5  Use differentiation to help determine the shape of the graph of a given function, including:
   ...
   b. when the graph is increasing or decreasing
